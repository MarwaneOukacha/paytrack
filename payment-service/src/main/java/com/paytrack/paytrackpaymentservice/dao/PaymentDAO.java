...
public class PaymentDAO {
    ...
    /**
     * Nombre de paiements par heure sur les dernières 24h.
     * Utilisé pour le graphique du dashboard.
     * Retourne une liste de Object[] : [heure (int), count (long)]
     */
    ...
    /**
     * Filtre multi-critères avec pagination manuelle.
     * Utilisé par GET /payments?status=X&accountId=Y&page=0&size=20
     */
    public List<Payment> findWithFilters(
        ...
        if (from      != null) jpql.append(" AND p.createdAt >= :from");
        if (to        != null) jpql.append(" AND p.createdAt <= :to");
    ...
    /**
     * Compte total pour la pagination du filtre multi-critères.
     */
    public long countWithFilters(
        ...
        StringBuilder jpql = new StringBuilder("SELECT COUNT(p) FROM Payment p WHERE 1=1");
